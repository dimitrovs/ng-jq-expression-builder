...
angular.module('jq.expression-builder', []).
directive('expressionBuilder', function() {
    function link(scope, element, attrs) {
        ...
        SubExpressionSelect.prototype.onSelect = function() {
            var subExpr = new SubExpressionElement(this.s$, {
                'leftComponent': this.leftType,
                'displayText': this.displayName,
                'expressionValue': this.expressionValue ? this.expressionValue : this.displayName,
                'rightComponent': this.rightType,
        ...
        function generateSystemSelectElements(s$) {
            ...
            var numberLiteral = new SubExpressionSelect({
                'returnType': 'NUMBER',
            ...
            var stringLiteral = new SubExpressionSelect({
                'returnType': 'TEXT',
        ...
        function SelectElementCollection(data, templates, s$) {
            ...
            this.templateGrouping = new SelectGrouping({
                'groupName': 'Templates',
                'values': templates
            }, s$, this.idMap, true);
        ...
        function SubExpressionUIComponent() {
            ...
            this.addExpressionElement = function(text, isOperator) {
                ...
                this.s$('.exprInner .input-small:visible:first').css({
        ...
        function TypeComponent(s$, type) {
            ...
            this.expressionElements = [];
            this.expandableInputs = [];
            this.expandableInputs = [this.addExpandableTypeInput({
                'returnType': type
            })];
        }
            ...
            if (typeof type === 'string') {
                ...
                $.each(type, _.bind(function(index, v) {
                    if (typeof v === 'string') {
                        ...
                        var ei = this.addExpandableTypeInput({
        ...
        function MultipleSizedListComponent(s$, type) {
            ...
            if (type) {
                if (typeof type === 'string') {
                    ...
                    var expandableInput = this.addExpandableTypeInput({
                        'returnType': 'NUMBER',
                        'isMultiSized': true,
                        'component': this
                    });
        ...
        var ExpressionBuilder = {
            ...
            init: function($elem, subExprsGrouped, options) {
                ...
                if (this.options.expressionURL) {
                    $.getJSON(this.options.expressionURL, function(data) {
                ...
                //s$('div.exprInner input:visible').eq(0).focus(); //doesn't work with expression-url
            },
            /**
             * Get expression.
             *
            ...
            saveAsTemplate: function(templateName) {
                ...
                var newTemp = _.extend(this.getJSON(), {
        ...
        attrs.expressionURL = attrs.expressionUrl; // Angular changes case
        element.on('eb-expression-complete', function (event,expression) {
            scope.ebExpressionComplete({expression:expression});
        });
        api.init.apply(element, [[],attrs]);
    }
    return {
        restrict: 'E',
        scope: {
            'ebExpressionComplete': '&',
        },
        templateUrl: 'expression-builder-template.html',
        link: link
    };
});